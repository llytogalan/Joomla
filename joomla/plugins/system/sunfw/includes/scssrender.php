		...
		switch ($type)
		{
			case 'menuroot':
				$arr = array(
					...
					'sticky-background-color' => 'false',
					'sticky-link-color' => 'false',
					'sticky-link-color-hover' => 'false',
					'sticky-background-hover-color' => 'false',
				);
			break;
			case 'menudropdown':
				$arr = array(
	...
	public function compile($styleID, $templateName, $s = "appearance")
	{
		...
		if ($s == "appearance")
		{
			...
			try
			{
				...
				// Compile Offcanvas Style
				if (isset($appearanceData['offcanvas']))
				{
					$offcanvases = $appearanceData['offcanvas'];
					$scss_content = '';

					// Section
					foreach ($offcanvases as $position => $offcanvas)
					{
						$scss_content .= "body.sunfw-offCanvas > .off-canvas-{$position} .offcanvas-content {";

						// Container
						if (isset($offcanvas['container']))
						{
							$container = $this->_prevarUse($offcanvas['container']);

							if (isset($container['background-image']) && $container['background-image'] != 'false')
							{
								if (preg_match($urlPattern, $container['background-image']))
								{
									$root = '';
								}
								else
								...
								$container['background-image'] = "'{$root}{$container['background-image']}'";
							}
							...
							if ($container)
							{
								$scss_content .= '@include offcanvas(' . implode(',', $container) . ');';
							}
						}

						// Heading
						if (isset($offcanvas['heading']))
						{
							$heading = $this->_prevarUse($offcanvas['heading']);
							$heading = $this->sectionParams($this->flatten($heading), 'heading');

							if ($heading)
							{
								$scss_content .= '@include offcanvas-heading(' . implode(',', $heading) . ');';
							}
						}

						// Content
						if (isset($offcanvas['content']))
						{
							$content = $this->_prevarUse($offcanvas['content']);
							$content = $this->sectionParams($content, 'content');

							if ($content)
							{
								$scss_content .= '@include content-offcanvas(' . implode(',', $content) . ');';
							}
						}

						// Link
						if (isset($offcanvas['link']))
						{
							$link = $this->_prevarUse($offcanvas['link']);
							$link = $this->sectionParams($link, 'link');

							if ($link)
							{
								$scss_content .= '@include offcanvas-link(' . implode(',', $link) . ');';
							}
						}

						// Default button
						if (isset($offcanvas['default-button']))
						{
							$default_button = $this->_prevarUse($offcanvas['default-button']);
							...
							if ($default_button)
							{
								$scss_content .= '@include btn-offcanvas-default(' . implode(',', $default_button) . ');';
							}
						}

						// Primary button
						if (isset($offcanvas['primary-button']))
						{
							$primary_button = $this->_prevarUse($offcanvas['primary-button']);
							...
							if ($primary_button)
							{
								$scss_content .= '@include btn-offcanvas-primary(' . implode(',', $primary_button) . ');';
							}
						}
					...
					$this->compileScss(array(), 'offcanvas', $templateName, $scss_content, $styleID);
				}
				...
				if (isset($appearanceData['module']))
				{
					...
					foreach ($modules as $key => $module_style)
					{
						...
						if (isset($module_style['container']))
						{
							...
							{
								if (preg_match($urlPattern, $module_container['background-image']))
								{
									$root = '';
								}
								else
								...
								$module_container['background-image'] = "'{$root}{$module_container['background-image']}'";
							}
				...
				if (isset($appearanceData['menu']))
				{
					...
					if (count($menus))
					{
					    $layoutItems = SunFwHelper::getLayoutItemList($styleID);
					}
					// Menu
					foreach ($menus as $key => $menu)
					{
						...
						if (isset($menu['root']))
						{
						    ...
						    if (count($layoutItems))
						    {
						        foreach ($layoutItems as $layoutItem)
						        {
						            if ($layoutItem->id == $key)
						            {
							...
							if ($sectionID !='')
							{
							    if (isset($root['sticky-background-color']) &&
							        isset($root['sticky-link-color']) &&
							        isset($root['sticky-link-color-hover']) &&
							        isset($root['sticky-background-hover-color']))
							    {
                                    $stickRoot = array();
                                    $stickRoot['sticky-background-color'] = $root['sticky-background-color'];
                                    $stickRoot['sticky-link-color'] = $root['sticky-link-color'];
                                    $stickRoot['sticky-link-color-hover'] = $root['sticky-link-color-hover'];
                                    $stickRoot['sticky-background-hover-color'] = $root['sticky-background-hover-color'];
                                    $stickyMenu = array('section_id' => $sectionID, 'menu_id' => 'menu_'.$key, 'style' => $stickRoot);
							    }
							}
						}
						...
						if (count($stickyMenu))
						{
						    ...
						    $sticky_menu_content .= "#sunfw_{$stickyMenu['section_id']}.sunfw-sticky-open {";
						    $sticky_menu_content .= '@include menu-root-sticky(' . $stickyMenu['style']['sticky-background-color'] . ');';
						    $sticky_menu_content .= '}';
						    $scss_content .= $sticky_menu_content;

						    $sticky_item_menu_content .= ".sunfw_{$stickyMenu['section_id']}.sunfw-sticky-open #{$stickyMenu['menu_id']} {";

						    $sticky_item_menu_content .= '@include menu-item-root-sticky(' . implode(',', array($stickyMenu['style']['sticky-link-color'], $stickyMenu['style']['sticky-link-color-hover'], $stickyMenu['style']['sticky-background-hover-color'])) . ');';

						    $sticky_item_menu_content .= '}';

						    $scss_content .= $sticky_item_menu_content;
						}
					}
		...
		elseif ($s == "layout")
		{
			...
			$data_layout_items = isset($data_layout['items']) ? $data_layout['items'] : array();
			if (count((array) $data_layout_items))
			{
			    foreach ((array) $data_layout_items as $key => $item)
			    {
			        if ($item['type'] == 'menu')
			        {

			            if (isset($item['settings']['icon-mobile-background-color']) && $item['settings']['icon-mobile-background-color'] != '')
			            {
			                $scss_content .= '#menu_' . $item['id'] . '_mobile {';
			                $scss_content .= 'background: ' . $item['settings']['icon-mobile-background-color'] . ';';
			                $scss_content .= 'border-color: ' . $item['settings']['icon-mobile-background-color'] . ';';
			                $scss_content .= '}';
			            }

			            $scss_content .= '#menu_' . $item['id'] . '_mobile > i{';

			            if (isset($item['settings']['icon-mobile-color']) && $item['settings']['icon-mobile-color'] != '')
			            {
			                $scss_content .= 'color: ' . $item['settings']['icon-mobile-color'] . ';';
			            }
			            if (isset($item['settings']['icon-mobile-size']) && $item['settings']['icon-mobile-size'] != '')
			            {
			                $scss_content .= 'font-size: ' . $item['settings']['icon-mobile-size'] . 'px;';
			            }
			            $scss_content .= '}';
			        }
			    }
