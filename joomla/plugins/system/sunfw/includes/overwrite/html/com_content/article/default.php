...
// Add necessary Open Graph meta data to support sharing to social network.
$app = JFactory::getApplication();
$doc = JFactory::getDocument();

$locale = explode('-', $doc->language);
...
$doc->addCustomTag('<meta property="og:title" content="' . htmlentities($doc->getTitle(), ENT_QUOTES) . '" />');
$doc->addCustomTag('<meta property="og:description" content="' . htmlentities(strip_tags(current(explode("\n", trim(preg_replace('/{(.*?)}/i', '', $this->item->introtext)), 2))), ENT_QUOTES) . '" />');
$doc->addCustomTag('<meta property="og:url" content="' . JUri::current() . '" />');
$doc->addCustomTag('<meta property="og:site_name" content="' . htmlentities($app->getCfg('sitename'), ENT_QUOTES) . '" />');

if (!empty($images->image_fulltext))
{
	$img = JUri::base() . $images->image_fulltext;
	$alt = empty($images->image_fulltext_alt)
		? empty($images->image_fulltext_caption)
			? $this->item->title
...
elseif (!empty($images->image_intro))
{
	...
	$alt = empty($images->image_intro_alt)
		? empty($images->image_intro_caption)
			? $this->item->title
...
if (isset($img))
{
	$doc->addCustomTag('<meta property="og:image" content="' . str_replace('https://', 'http://', $img) . '" />');
	$doc->addCustomTag('<meta property="og:image:secure_url" content="' . str_replace('http://', 'https://', $img) . '" />');
	$doc->addCustomTag('<meta property="og:image:alt" content="' . htmlentities($alt) . '" />');
}
...
<div class="item-page<?php echo $this->pageclass_sfx; ?>" itemscope itemtype="https://schema.org/Article">
	<meta itemprop="mainEntityOfPage" content="<?php echo JUri::current(); ?>">
