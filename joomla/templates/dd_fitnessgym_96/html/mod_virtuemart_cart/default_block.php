...
<?php
    ...
    if (preg_match('/[\.\d' . $decimalSymbol . $thousandsSeparator . ']+/', trim($product['subtotal_with_tax']), $matches)) {
        $str = str_replace($thousandsSeparator, '', $matches[0]) / $product['quantity'];
        ...
        if (count($parts) > 1) {
            $dec = round(0 . $decimalSymbol . $parts[1], $decimals);
        }
        $price .= substr($dec, 1);
...
?>
 ...
 bd-icon-60 bd-icon "></span>
    ...
    <span class=" bd-container-73 bd-tagstyles bd-bootstrap-btn bd-btn-warning">
        <?php echo $totalPrice; ?>
    </span>
</div>
	
		<a href="<?php echo $cartHref; ?>" class=" btn   btn-warning">
    <?php echo $cartText; ?>
</a>
	
		<a href="<?php echo JRoute::_ ('index.php?option=com_virtuemart&view=cart&task=checkout_task'); ?>" class=" btn   btn-warning">
